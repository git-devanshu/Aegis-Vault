...
export default function Popup({isOpen, onClose, title, borderColor=theme.success, children, bg=theme.cardBg, takeFullHeight=false}) {
    const {DISPLAY} = useLanguage();

    return (
        <Modal isOpen={isOpen} onClose={onClose} isCentered autoFocus={false} returnFocusOnClose={false} trapFocus={false}>
            <ModalOverlay bg='rgba(0,0,0,0.6)'/>
            <ModalContent bg={bg} color={theme.text} border={`1px solid ${theme.border}`} borderTop={`10px solid ${borderColor}`} borderRadius={theme.radius} height={takeFullHeight ? '95%' : 'fit-content'} width={{base:'92%', sm:'85%', md:'70%', lg:'500px'}} maxWidth='500px'>
