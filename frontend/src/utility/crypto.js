// generate random salt for auth hash derivation
// @returns : salt - string
...
// @params : plainInput - string, saltBase64 - string
// @returns : hash - string
export async function createHash(plainInput, saltBase64) {
    const salt = base64ToArray(saltBase64);
    const encoder = new TextEncoder();

    const keyMaterial = await crypto.subtle.importKey(
        'raw',
        encoder.encode(plainInput),
        'PBKDF2',
        false,
        ['deriveBits']
    );
